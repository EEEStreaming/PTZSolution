                ...
                if ((m_Socket != null) && (m_Socket.Connected))
                {
                    ...
                    {                        
                        ...
                        if (bytesRead > 0)
                        {
                            ...
                            {
                                PTZLogger.Log.Debug("{0},{1}, Raw msg : {2}", m_Name, m_Host, BitConverter.ToString(receivedByteArray, 0, bytesRead));
                            }

                            // check length max and move received byte array in global byte array
                            if(lenByteArray+bytesRead >= MAX_BYTEARRAY_LENGTH)
                            {
                                PTZLogger.Log.Error("{0},{1}, Buffer byteArray overflow (waiting buffer:{2}, bytesRead:{3}, total:{4}, max allowed:{5}) BUFFER CLEARED", m_Name, m_Host, lenByteArray, bytesRead, lenByteArray+bytesRead,MAX_BYTEARRAY_LENGTH);
                                lenByteArray = 0;
                            ...
                            {
                                PTZLogger.Log.Debug("{0},{1}, startPos({2}) > bytesRead({3}) - 2", m_Name, m_Host, startPosition, bytesRead);
                                continue;
                            }
                            ...
                            {
                                PTZLogger.Log.Debug("{0},{1}, msgLen({2})> bytesRead({3}) - startPos({4})", m_Name, m_Host, msgLen, bytesRead,startPosition);
                                continue;
                            }
                    ...
                    catch (SocketException exception)
                    {
                        if (exception.ErrorCode != 10004)//ErrorCode 10004: A blocking operation was interrupted by a call to WSACancelBlockingCall.
                            PTZLogger.Log.Error(exception,"{0},{1}, Error 10004 when received data", m_Name, m_Host);
                        //else Nothing to do, it's just a socket close.                       
                        return m_FreeSocket;
                    }
                    catch (Exception ex)
                    {
                        PTZLogger.Log.Error(ex, "{0},{1}, Error when received data", m_Name, m_Host);
